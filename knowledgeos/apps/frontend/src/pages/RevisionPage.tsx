...
/**
 * RevisionPage — Premium Spaced Repetition Study Deck.
 * Uses 3D CSS card flipping and rating triggers driven by the SM-2 algorithm.
 */
...
const recallButtons = [
  ...
  { val: 5, label: '5', hoverColor: 'bg-primary-container text-white scale-110 shadow-lg primary-glow' },
];
...
export function RevisionPage() {
  ...
  return (
    <div className="animate-fade-in space-y-6 select-none flex-grow flex flex-col justify-start max-w-5xl mx-auto w-full">
      {/* Page Header & Stats Dashboard */}
      <div className="flex flex-col gap-md">
        <div className="flex justify-between items-end flex-wrap gap-md">
          <div>
            <h2 className="font-display-lg text-headline-lg text-on-surface">Spaced Repetition</h2>
            <p className="font-body-md text-xs text-on-surface-variant">Daily session to strengthen recall parameters of vector documents.</p>
          </div>
          <div className="flex gap-md">
            <div className="glass-panel px-lg py-md rounded-xl flex items-center gap-md">
      ...
      <div className="flex-1 flex items-center justify-center py-xl min-h-[360px]">
        {isDueLoading ? (
          <div className="rounded-2xl border border-outline-variant bg-surface-container p-12 text-center h-80 flex flex-col justify-center items-center w-full max-w-[720px] aspect-[16/9]">
            <div className="skeleton h-8 w-1/3 mb-4" />
            <div className="skeleton h-4.5 w-1/2" />
          </div>
        ) : currentItem ? (
          <div className="card-container w-full max-w-[720px] aspect-[16/9]">
            <div
              className={`card-inner cursor-pointer h-full ${flipped ? 'flipped' : ''}`}
              onClick={() => setFlipped((f) => !f)}
            >
              {/* Front Face */}
              <div className="card-front glass-panel p-3xl flex flex-col justify-center items-center text-center">
                ...
                <span className="material-symbols-outlined text-primary-fixed-dim opacity-40 mb-lg text-4xl block">
                  psychology
                </span>
              ...
              {/* Back Face */}
              <div className="card-back glass-panel p-xl flex flex-col justify-between">
                ...
                <div className="pt-xl border-t border-outline-variant/30 mt-auto">
                  ...
                  <div className="flex justify-center gap-sm flex-wrap">
                    {recallButtons.map((btn) => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-lg bg-surface-container border border-outline-variant flex items-center justify-center font-bold transition-all text-xs cursor-pointer ${btn.hoverColor}`}
                      >
                        {btn.label}
                      </button>
                    ))}
          ...
          <div className="rounded-2xl border border-dashed border-outline-variant p-12 text-center h-80 flex flex-col justify-center items-center w-full max-w-[720px] aspect-[16/9] bg-surface-container/20">
      ...
      <div className="flex justify-center items-center gap-xl text-xs font-label-sm text-on-surface-variant flex-wrap">
        <div className="flex items-center gap-xs">
          <kbd className="px-2 py-1 bg-surface-container border border-outline-variant rounded text-[10px] font-mono">SPACE</kbd>
          <span>Flip card</span>
        </div>
        <div className="flex items-center gap-xs">
          <kbd className="px-2 py-1 bg-surface-container border border-outline-variant rounded text-[10px] font-mono">0-5</kbd>
