// apps/frontend/src/pages/StatusPage.tsx
/**
 * StatusPage — Premium System Health Dashboard.
 * Reads live health details from backend telemetry APIs.
 */
...
export function StatusPage() {
  ...
  const { data: syncStatus, refetch: refetchSync, isFetching: isSyncFetching } = useQuery<SyncStatus>({
    ...
    <div className="animate-fade-in space-y-8 select-none max-w-5xl mx-auto w-full">
      ...
      <section className="flex justify-between items-end flex-wrap gap-md">
        <div>
          ...
          <div className="flex items-center gap-sm mt-xs">
            <span className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(78,222,163,0.5)] status-pulse ${isGlobalActive ? 'bg-secondary' : 'bg-error'}`}></span>
      ...
      <div className="grid grid-cols-12 gap-lg">
        ...
        <div className="col-span-12 lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-md">
          {/* Gateway Status */}
          <div className="glass-panel p-md rounded-xl flex flex-col gap-sm justify-between min-h-[160px] bg-surface-container">
          ...
          <div className="glass-panel p-md rounded-xl flex flex-col gap-sm justify-between min-h-[160px] bg-surface-container">
            ...
            <div className="mt-auto h-1 w-full bg-surface-container-highest rounded-full overflow-hidden relative">
              {isMLActive && (
                <div className="absolute h-full bg-primary w-1/3 animate-[skeleton-shimmer_2s_infinite_linear]" />
              )}
            </div>
          </div>
        </div>

        {/* Latency History Telemetry */}
        <div className="col-span-12 lg:col-span-4 glass-panel p-lg rounded-xl flex flex-col justify-between bg-surface-container min-h-[160px] text-xs">
          <div>
            <h4 className="font-headline-lg text-sm font-semibold">Gateway Request Latency</h4>
        ...
        {/* Sync logs heartbeat audit reports */}
        <div className="col-span-12 glass-panel p-lg rounded-xl bg-surface-container text-xs">
          <div className="mb-md border-b border-outline-variant/30 pb-3">
            <h4 className="font-headline-lg text-sm font-semibold">Sync Log Ingestion Pipeline</h4>
