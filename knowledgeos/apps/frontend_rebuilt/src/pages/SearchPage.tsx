...
export default function SearchPage() {
  ...
  // Automatically select first result on search completion
  useEffect(() => {
  ...
  // Highlight matching words in snippets
  const renderHighlightedText = (text: string, q: string) => {
    ...
    return (
      <>
        {parts.map((part, idx) =>
          regex.test(part) ? (
            <span key={idx} className="bg-primary/20 text-primary font-semibold px-0.5 rounded">
              {part}
            </span>
          ) : (
            part
  ...
  return (
    <div className="flex flex-col items-center select-none flex-1 min-h-[calc(100vh-100px)]">
      {/* Search Header */}
      <section className="w-full max-w-3xl relative z-20">
        <div className="text-center mb-6">
          <h1 className="font-display-lg text-headline-lg text-on-surface mb-xs font-semibold">Search Intelligence</h1>
          <p className="font-body-md text-on-surface-variant text-sm">Analyze across vectors, documents, and neural graphs.</p>
        </div>
        
        {/* Input area */}
        <div className="relative w-full group">
          <div className="flex items-center bg-surface-container border border-outline-variant rounded-xl p-4 transition-all duration-300 focus-within:ring-2 focus-within:ring-primary/20">
            <span className="material-symbols-outlined text-primary mr-md">search</span>
            <input
              ref={inputRef}
              className="bg-transparent border-none outline-none w-full text-on-surface text-sm placeholder:text-on-surface-variant/40"
            ...
            <div className="flex gap-xs items-center ml-auto">
              <kbd className="font-code text-[11px] bg-surface-container-lowest px-2 py-0.5 rounded border border-outline-variant/30 text-on-surface-variant font-mono">⌘</kbd>
              <kbd className="font-code text-[11px] bg-surface-container-lowest px-2 py-0.5 rounded border border-outline-variant/30 text-on-surface-variant font-mono">K</kbd>
            </div>
          </div>

          {/* Spotlight history */}
          {focused && recentSearches.length > 0 && (
            <div className="absolute top-[calc(100%+8px)] left-0 w-full glass-panel rounded-xl overflow-hidden shadow-2xl z-30 animate-fade-in bg-surface-container-lowest">
              <div className="p-sm border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-high/40">
                ...
                <button
                  className="font-label-sm text-primary hover:brightness-110 px-md text-[10px] font-bold cursor-pointer"
                  onClick={() => {
              ...
              <div className="py-1">
                {recentSearches.map((historyQuery, index) => (
                  ...
                  >
                    ...
                    <span className="flex-1 text-xs text-on-surface">{historyQuery}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Results view split */}
      {submittedQuery && (
        <section className="w-full mt-8 grid grid-cols-12 gap-lg overflow-hidden items-stretch flex-1">
          {/* Matches column list */}
          <div className="col-span-12 lg:col-span-7 flex flex-col gap-md pr-1 overflow-y-auto max-h-[550px]">
            {isLoading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="skeleton h-24 w-full bg-white/5 rounded-xl animate-pulse" />
                ))}
              </div>
              ...
              data.results.map((res, index) => (
                ...
                >
                  <div className="flex justify-between items-start mb-sm gap-2">
                    ...
                    <div className="bg-secondary/10 border border-secondary/20 text-secondary px-2 py-0.5 rounded text-[10px] font-bold flex-shrink-0">
              ...
              <div className="text-center p-12 glass-panel rounded-xl text-on-surface-variant">
                ...
                <span className="text-xs">No vector matches found for "{submittedQuery}". Try refining parameters.</span>
              </div>
            )}
          </div>

          {/* Details visual previewer */}
          <div className="col-span-12 lg:col-span-5 flex flex-col bg-surface-container border border-outline-variant rounded-xl overflow-hidden min-h-[300px]">
            {selectedResult ? (
              <div className="p-xl flex-1 flex flex-col justify-between space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="font-label-sm text-label-sm px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded-full text-[10px] font-bold uppercase">
                      Concept Excerpt
                    </span>
                  ...
                  <div className="flex flex-wrap gap-xs">
                    <span className="text-[10px] bg-white/5 border border-outline-variant/30 px-2 py-0.5 rounded font-mono text-on-surface-variant">
                ...
                <div className="flex-1 min-h-[150px] bg-surface-container-low border border-outline-variant/40 p-4 rounded-xl overflow-y-auto font-sans leading-relaxed text-xs text-on-surface">
                  {renderHighlightedText(selectedResult.chunkContent, submittedQuery)}
                </div>

                <div className="pt-4 border-t border-outline-variant/20 text-[10px] text-on-surface-variant font-mono">
